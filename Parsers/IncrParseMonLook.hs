...
toQ (Shif p) = \k h -> Shift $ toQ p k h
...
feed ss p = case p of
                  (Sus nil cons) -> case ss of
                      Nothing -> p -- no more info, stop feeding
                      Just [] -> feed (Just []) nil -- finish
                      Just (s:_) -> feed ss (cons s)
                  (Shift p') -> Shift (feed (fmap (drop 1) ss) p')
...
lookNext f = Look (if f Nothing then ok else empty) (\s -> 
