...
module Parsers.Incremental (-- Parsers 
                            Parser(..),
                            recoverWith, symbol, eof, lookNext, testNext, oops,
                            runParser,
                            -- Processes
                            Process, 
                            mkProcess, evalL, evalR, feedZ, pushSyms, pushEof, run,
                            -- debug
                            profile, atRight
                            ) where
...
instance Show (Steps s r) where
    ...
    show (Shift p) = ">" ++ show p
    show (Sh' p) = "'" ++ show p
    show (Dislike msg p) = show msg ++ "?"  ++ show p
    show (Fail) = "0"
    show (Sus _ _) = "..."
...
atRight :: (forall b. Steps s b ->c) ->(Zip s a ->c)
atRight f (Zip _ _ y) = f y
