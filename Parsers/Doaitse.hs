...
class (Applicative a, Alternative a) => AA a where
  (<$$>)    ::   (b -> a)    -> p b   ->   p a
  f <$$> p   =  pure f <*> p
...
class AA p => ExtApplicative p st where
  (<*)      ::  p  a          -> R st b   ->   p  a
  (*>)      ::  R  st b          -> p    a   ->   p  a
  (<$$)      ::  a            -> R st b   ->   p  a
...
instance Applicative (P_f s state) => Monad (P_f s state) where
   P_f p >>= pv2q  = P_f  (  \ k st ->  
                             let  steps = p (unP_f (pv2q pv) k) st
                                  pv = fst (eval steps)
                             in  Apply snd steps
                          ) 
   return     = pure
...
instance Show (Steps   r) where
 show (Fail  _ l  )     = "Fail " ++ show (l undefined)
 show (Step  p  l )     = "Step " ++ show l
 show (Apply f l  )     = "Apply " ++ show l
 show (End_f_ _    )     = "End"
-}