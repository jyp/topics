...
fairCross f xs ys = concat (diags [[f x y | x <- xs] | y <- ys])

diags = diags' []
    where diags' xss (ys : yss) = map head xss : diags' (ys : fnn (map tail xss)) yss
          diags' []  []         = []
          diags' xss []         = map head xss : diags' (     fnn (map tail xss)) []
          fnn = filter (not . null)
