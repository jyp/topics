...
\textmeta{API section? Full example? Make sure Online and Incremental parsing are properly introduced.}
...
The abstract syntax tree (AST) of the source code is available at
all times, kept up to date as the user types: this allows all 
syntax-dependent functions to depend on it, which in turn ensures consistency
between them.
