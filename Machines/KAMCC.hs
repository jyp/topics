...
type Sym = String
data Term = Var Sym | Lam Sym Term | App Term Term | Con String | CC
--  deriving Show
...
type Value = Closure
data Closure = Term :+ Env | Cont Stack
  deriving Show
type Env = [(Sym,Value)]
...
step (Lam x t   :+ rho, [])  = Nothing
step (Lam x t   :+ rho, v:s) = Just (t :+ ((x,v):rho), s)
step (App t1 t2 :+ rho, s)   = Just (t1 :+ rho, (t2 :+ rho):s)
step (CC        :+ rho, e:s) = Just (e, Cont s:s)
step (Cont gamma      , e:s) = Just (e, gamma)
step _ = Nothing
